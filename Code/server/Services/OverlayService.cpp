...
{
    auto [canceled, reason] = m_world.GetScriptService().HandleChatMessage(Script::Player(acMessage.pPlayer->GetId(), *acMessage.pPlayer->GetCharacter(), m_world), acMessage.Packet.ChatMessage);
    if (canceled)
        return;
